...
# cluster info
st.metric("Number of Rows in the Cluster", f"{len(df_clustered[df_clustered['cluster'] == selected_cluster])}")

cluster_count = len(df_clustered[df_clustered['cluster'] ==selected_cluster])
cluster_per = cluster_count * 100 / len(df_clustered)
st.metric("Percentage the Cluster Covers", f"{cluster_per:.2f}%")
